...
def add_user_days(uid, days):
 ...
 cur.execute("INSERT OR REPLACE INTO users (user_id, expires_at) VALUES (?, ?)", (uid, n_exp_str))
 conn.commit()
 conn.close()
 return n_exp_str
...
def admin_cb(call):
 ...
 if call.data == "admin_stats":
  ...
  msg = bot.send_message(call.message.chat.id, "✍️ Введи Telegram ID пользователя, которому хочешь выдать доступ:")
...
def admin_get_id(m):
 ...
 markup = types.InlineKeyboardMarkup()
 markup.add(types.InlineKeyboardButton("🚀 1 Месяц", callback_data=f"adm_give_{target_id}_30"))
 markup.add(types.InlineKeyboardButton("🔥 3 Месяца", callback_data=f"adm_give_{target_id}_90"))
 markup.add(types.InlineKeyboardButton("💥 6 Месяцев", callback_data=f"adm_give_{target_id}_180"))
 markup.add(types.InlineKeyboardButton("👑 1 Год", callback_data=f"adm_give_{target_id}_365"))
 markup.add(types.InlineKeyboardButton("♾ Навсегда", callback_data=f"adm_give_{target_id}_9999"))
 bot.send_message(m.chat.id, f"⏳ Выбери время подписки для ID `{target_id}`:", reply_markup=markup, parse_mode="Markdown")
...
def admin_confirm_give_cb(call):
 ...
 bot.send_message(call.message.chat.id, f"✅ Подписка на {days} дней успешно добавлена для ID `{t_id}`!", parse_mode="Markdown")
 key, country = get_happ_config()
 if key:
  try:
   text = f"🎉 Администратор активировал вам подписку на **{days} дней**!\n📍 Узел: {country}\n\nВаш ключ для Happ:\n`{key}`"
   bot.send_message(t_id, text, parse_mode="Markdown")
   bot.send_message(call.message.chat.id, f"🚀 Ключ автоматически отправлен пользователю в чат!")
...
def pay_select_cb(call):
 ...
 markup = types.InlineKeyboardMarkup()
 markup.add(types.InlineKeyboardButton("⭐ Telegram Stars", callback_data=f"buy_stars_{tariff}"))
 markup.add(types.InlineKeyboardButton("💳 Банковская карта / СБП (Вручную)", callback_data=f"buy_manual_{tariff}"))
 bot.send_message(call.message.chat.id, "💳 **Выберите способ оплаты:**", reply_markup=markup, parse_mode="Markdown")
...
def payment_cb(call):
 ...
 method = parts
 tariff = parts
 t_map = {"1m": ("1 мес", 50, "50 руб", 30), "3m": ("3 мес", 85, "85 руб", 90), "6m": ("6 мес", 150, "150 руб", 180), "1y": ("1 год", 250, "250 руб", 365), "inf": ("Навсегда", 500, "500 руб", 9999)}
