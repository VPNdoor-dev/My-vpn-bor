...
def get_trial_days():
    ...
    limit = datetime.date(2026, 10, 15)
    return 5 if today <= limit else 3
...
def get_total_users():
    ...
    tr = cur.fetchone()
    conn.close()
    return tot, tr def check_trial(uid):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute(
...
def add_user_days(uid, days):
    ...
    if res and res[0]:
        ...
        base_date = (
            cur_exp if cur_exp >= today else today
        )
    else:
        base_date = today

    n_exp = base_date + datetime.timedelta(
        days=days
    )
...
def check_user_status(uid):
    ...
    return "🔴 Не активна" def get_main_keyboard(uid):
...
def get_happ_config():
    try:
        ...
        # Короткий таймаут спасает от зависания
        ...
        if ips:
            ...
            ip_addr = ch[1] if ch[1] else "127.0.0.1"
            m = "chacha20-ietf-poly1305:password123"
            b64 = base64.b64encode(
                m.encode("utf-8")
            ).decode("utf-8")
            key = (
                f"ss://{b64}@{ip_addr}:443"
                f"#DoorVPN-{ip_addr}"
            )
            return key, ip_addr
...
def text(m):
    ...
    if m.text == "📊 Тарифы и Оплата":
        ...
        markup.add(
            ...
            types.InlineKeyboardButton(
                "🎫 Выдать доступ",
                callback_data="admin_give_trial",
            ),
        )
        bot.send_message(
            m.chat.id,
...
def payment_cb(call):
    ...
    t_map = {
        "buy_1m": ("1 мес", 50, 30),
        "buy_3m": ("3 мес", 120, 90),
        "buy_6m": ("6 мес", 220, 180),
        "buy_1y": ("1 год", 400, 365),
        "buy_inf": ("Навсегда", 1000, 9999),
    }
...
def success_pay(message):
    ...
    p = pay.invoice_payload
    d = int(p.split("_")[-1])
    uid = message.from_user.id
    add_user_days(uid, d)
    bot.send_message(
        message.chat.id, "⏳ Подключаю..."
    )
    key, country = get_happ_config()
    if key:
        bot.send_message(
